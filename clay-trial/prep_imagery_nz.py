...
class ImageryPreprocessor:
    def __init__(self, tif_path, metadata_path, lat, lon, platform="linz", device="cpu"):
    ...
    def get_datacube(self):
        ...
        # Handle missing descriptions - default to RGB + NIR
        if descriptions and descriptions[0]:
        ...
        # Read only first 4 bands, skip band 5
        pixels_data = dataset.read([1, 2, 3, 4]).astype(np.float32)
        gsd = dataset.res[0]
